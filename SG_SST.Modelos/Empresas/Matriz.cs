...
namespace SG_SST.Models.Empresas
{
    ...
    public class Matriz
    {
        ...
        {
            string cadena = " PK_Matriz: " + PK_Matriz +
                            " Descripcion: " + Descripcion +
                            " Empresa: " + ((Empresa == null) ? FK_Empresa.ToString() : Empresa.Razon_Social);
            if (ElementosMatriz != null)
            foreach (ElementoMatriz elemento in ElementosMatriz)
                cadena = cadena + " ElementoMatriz:{ " + elemento.ToString() + "}";
            return cadena;
        }
