            ...
            success: function (result) {
                ...
                if (result) {
                    ...
                    swal({
                        type: 'success',
                        title: 'Estimado Usuario',
                        text: 'Planeación Inspección Eliminada',
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result) {
                    ...
                    swal({
                        type: 'success',
                        title: 'Estimado Usuario',
                        text: 'Fecha final plan de acción generada, se obtiene sumando la fecha de la Planeación de Inspección más la mayor prioridad en días de las condiciones inseguras seleccionadas.',
                        confirmButtonColor: '#7E8A97',
            ...
            error: function (result) {
                swal({
                    type: 'error',
                    title: 'Estimado Usuario',
                    text: 'Se presentó un error, intente nuevamente',
                    confirmButtonColor: '#7E8A97'
                });
            }
        });
...
//Cargar el nombre del archivo en un label
function obtenerNombreArchi(){
    var texto = document.getElementById('Evidencia').files[0].name;
    $("#fichero").text(texto)
}
...
function postGuardar() {
    this.launchGuardar = function () {
        ...
        if (Condiciones.length > 0) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.probar == false) {
                        ...
                        swal({
                            title: "Estimado Usuario",
                            text: "Se ha generado Accción Correctiva No. :" + data.Idaccionmostrar + " , Consultar el Modulo de Acciones Correctivas para completar el diligenciamiento",
                            type: "success",
                            confirmButtonColor: '#7E8A97',
                            confirmButtonText: "Aceptar",
            ...
            swal({ title: "Estimado Usuario", text: "Debe seleccionar mínimo un Ítem de la lista", type: "warning", confirmButtonColor: '#7E8A97' })
        }
    }
}
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.Data == 0) {
                    ...
                    swal({ title: "Estimado Usuario", text: "El Ítem seleccionado fue registrado anteriormente", type: "warning", confirmButtonColor: '#7E8A97' })
                    OcultarPopupposition();
                }
            },
            error: function (result) {
...
function FormularioGenerarInspeccion() {
    $("#generarcondicion").validate({
        ...
        messages: {
            ...
            ListaEHM: { required: "Seleccione un Ítem" },
            fechaplaninspeccion: {
                required: "Campo no puede estar vacío"
            },
            hora: {
    ...
    if (IDOTRO == $idtipoPeligro.val()) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result) {
                    $("#FK_Clasificacion_De_Peligro").empty();
                    $("#FK_Clasificacion_De_Peligro").append("<option value=\"\">--Seleccionar Clasificación Peligro--</option>");
