...
$(document).ready(function () {
    ...
    $("#IdContingencia").on('change', function () {
        ...
        if (valorContingencia == '4' || valorContingencia == '5' || valorContingencia == '13' || valorContingencia == '14') {
            $('#tex-ley').show();
        } else {
            $('#tex-ley').hide();
        }
        if (valorContingencia == '4' || valorContingencia == '13')
        {
        ...
        if (textoContingencia == 'Licencia de maternidad' && sx == 'M') {
            ...
            $("#IdContingencia").val("");
        } else if (textoContingencia == 'Licencia de Maternidad Parto Múltiple' && sx == 'M') {
            swal("Atención", 'Esta contingecia no puede ser asignada a este Trabajador.')
            $("#IdContingencia").val("");
        } else if (textoContingencia == 'Licencia de Maternidad Nacimiento Prematuro' && sx == 'M') {
            swal("Atención", 'Esta contingecia no puede ser asignada a este Trabajador.')
            $("#IdContingencia").val("");
    ...
    $('#FechaFin').on("change", function (e) {        
        ...
        if (fecha2.getTime() < fecha1.getTime()) {
            swal("Atención", 'La Fecha de finalización no puede ser menor a la inicial');
            return false;
        }
        ...
        if (f1 == '' || f2 == '') {
            ...
            $.ajax({
                ...
                if (response != undefined && response.status == 'Success') {
                    ...
                    if (diasa != 126 && idTipoConting == 4) {
                        $('#DiasAusencia').val("");
                        $('#FechaFin').val('');
                        swal("Atención", 'La Licencia de maternidad debe tener exactamente 126 días de incapacidad, actualmente esta ingresando ' + diasa + ' días, Verifique las fechas');
                        OcultarPopupposition();
                        return false;
                    } else if (diasa != 8 && idTipoConting == 5) {
                        $('#DiasAusencia').val("");
                        $('#FechaFin').val('');
                        swal("Atención", 'La Licencia de paternidad debe tener exactamente 8 días de incapacidad, Verifique las fechas');
                        OcultarPopupposition();
                        return false;
                    } else if (diasa != 5 && idTipoConting == 7) {
                        $('#DiasAusencia').val("");
                        $('#FechaFin').val('');
                        swal("Atención", 'La Licencia de luto debe tener exactamente 5 días de incapacidad, Verifique las fechas');
                        OcultarPopupposition();
                        return false;
                    } else if (diasa != 140 && idTipoConting == 13) {
                        $('#DiasAusencia').val("");
                        $('#FechaFin').val('');
                        swal("Atención", 'La Licencia de Maternidad Parto Múltiple debe tener exactamente 140 días de incapacidad, actualmente esta ingresando ' + diasa + ' días, Verifique las fechas');
                        OcultarPopupposition();
                        return false;
                    } else if ((diasa <= 126 || diasa > 217) && idTipoConting == 14) {
                        $('#DiasAusencia').val("");
                        $('#FechaFin').val('');
                        swal("Atención", 'La Licencia de Maternidad Nacimiento Prematuro, debe ser superior a 126 días sin superar los 217 días, actualmente esta ingresando ' + diasa + ' días, Verifique las fechas por favor.');
                        OcultarPopupposition();
                        return false;
                    }
                    ...
                    swal("Estimado Usuario", '"El afiliado ya presenta ausentismos registrados en las fechas ingresadas.');
                    $('#FechaFin').val('');
                    OcultarPopupposition();
                    return false;
                }
            ...
            if (idTipoConting == '9') {
                if (f1 != f2) {
                    $('#FechaFin').val('');
                    swal("Atención", 'En este tipo de contingecia la Fecha Inicio y Fin deben ser iguales.');
                    return false;
                }
            } else {
                $('#FechaFin').val('');
    ...
    $('#FechaInicio').on("change", function (e) {
        ...
        $('#FechaFin').attr("disabled", false);
        //var fechaMayorActual = document.getElementById("FechaInicio").value;
        //var fechaMayorActualConvert = new Date(fechaMayorActual.split('/')[2], fechaMayorActual.split('/')[1] - 1, fechaMayorActual.split('/')[0]);
        //var fechaActuallll = new Date();
        //if (fechaActuallll.getTime() < fecha1444.getTime()) {
        //    $('#FechaFin').val("");
        //    $('#FechaFin').html("");
        //    $('#FechaFin').attr("disabled", true);
        //    swal("Atención", 'Fecha inicial  no puede ser mayor a la  fecha actual');
        //    return false;
        //}
        var idTipoContingMat = $('#IdContingencia').val();
        if (idTipoContingMat == 4)
        {
            $('#FechaFin').val('');
            $('#FechaFin').attr("disabled", true);
            var fechaDefinalizacion = new Date();
            var fech1 = document.getElementById("FechaInicio").value;
            var fecha = new Date(fech1.split('/')[2], fech1.split('/')[1] - 1, fech1.split('/')[0]);
            var fechaActuallll = new Date();
            if (fechaActuallll.getTime() < fecha.getTime()) {
                $('#FechaFin').val("");
                $('#FechaFin').html("");
                $('#FechaFin').attr("disabled", true);
                swal("Atención", 'Fecha inicial  no puede ser mayor a la  fecha actual');
            ...
            var addTime = 125 * 86400; //Tiempo en segundos
            ...
            fechaDefinalizacion = dia + "/" + mes + "/" + anio;
            $('#FechaFin').val(fechaDefinalizacion);
            swal("Atención", 'Conforme a la Ley 1822 de 2017 la licencia de maternidad tiene una duración de 18 semanas (126 días), por lo cual el sistema lo calcula automáticamente a partir de la fecha de inicio de la licencia');
        } else if (idTipoContingMat == 13) {
            $('#FechaFin').val('');
            $('#FechaFin').attr("disabled", true);
            var fechaDefinalizacion = new Date();
            var fech1 = document.getElementById("FechaInicio").value;
            ...
            //var tiempo = 126;
            var addTime = 139 * 86400; //Tiempo en segundos
            fecha.setSeconds(addTime); //Añado el tiempo                                            
            var dia = fecha.getDate();
            if (dia < 10) {
                dia = "0" + dia
            ...
            fechaDefinalizacion = dia + "/" + mes + "/" + anio;
            $('#FechaFin').val(fechaDefinalizacion);
            swal("Atención", ' Conforme a la Ley 1822 de 2017 la licencia de maternidad tiene una duración de 2 semanas adicionales a las 18 semanas iniciales para un total de 20 semanas (140 días), por lo cual el sistema lo calcula automáticamente a partir de la fecha de inicio de la licencia');
        }
        ...
        if (fechaActual.getTime() < fecha1.getTime()) {
            $('#FechaInicio').val("");
            swal("Atención", 'La Fecha inicial  no puede ser mayor a la  fecha actual');
            return false;
        }
        ...
        if (f1 == '' || f2 == '') {
            $('#DiasAusencia').val('');
            alert(f1, f2 + 'por aqui como hice');
            //swal("Atención", 'Debe seleccionar las Fechas Inicio y Fin Fin fin.');
            OcultarPopupposition();
            e.stopPropagation();
        }
        else if (fecha1.getTime() < fecha2.getTime() && idTipoConting != '9') {
    ...
    $('#HoraFinalizacion').on('change', function () {
        ...
        if (horaspermiso > catidadTotal) {
            $('#HoraInicio').val('');
            $('#HoraFinalizacion').val('');
            swal("Atención", 'El permiso no debe exceder las 8 horas laborables por día.');
            return false;
        } else {
            PopupPosition();
            CalcularDias(f1, f2, idTipoConting, horaInicio, horaFin, 'DiasAusencia', 'FechaInicio', 'FechaFin');
    ...
    $("#NuevaAusencia").click(function (event) {
        ...
        if (DiasAusencia == "") {
            swal("Atención", 'El campo días de ausencia esta vacio, por favor verifique las fechas o el campo salario');
            $("#FechaFin").val("");
            $("#FechaInicio").val("");
            return false;
        }
...
function CalcularDiasHabiles(f1, f2, idTipoConting, horaIni, horaFn, idDiasAusencias, idFechaI, idFechaF) {
    var fecha1 = new Date(f1.split('/')[2], f1.split('/')[1] - 1, f1.split('/')[0]);
    var fecha2 = new Date(f2.split('/')[2], f2.split('/')[1] - 1, f2.split('/')[0]);
