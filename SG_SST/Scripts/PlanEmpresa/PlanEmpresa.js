							...
							var color = "";
								for (var i = 1; i <= 12; i++) {
									if (i == mes) {
										...
										tHtml += '<td '+class_css_btxt+' class="CellWithComment" style="cursor:pointer;" onclick="ObtenerActividad(' + val.pk_id_actividad + ')"><span class="CellComment">Actividad : '+ val.Actividad +' <br/> Fecha Reprogramada : '+val.FechaReProg+'<br>Fecha Ejecutada : '+val.FechaEje+'</span>';
...
function ObtenerActividad(pkActividad) {
    $.ajax({
        ...
        $('#myModal1').modal('show');
            ...
            $("#ObjetivosMetas").val(result.ObjetivosMetas);
