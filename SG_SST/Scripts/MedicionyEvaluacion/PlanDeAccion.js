...
function AdicionarActividad(id,Fk_Id_ModuloPlanAccion, Fk_Plan_Inspección, Fk_Id_Actividad, Num_Actividad) {
    ...
    if (Fk_Id_ModuloPlanAccion==1) {

        var fechaahora = new Date(Date.now());
        fechaahora.setHours(0, 0, 0, 0);
        var fechaFinMod1 = fechaFinalizacion;

        var splitfecha1 = fechaFinMod1.split("/");
        var splitfecha2 = fechaFinMod1.split("-");
...
function NoEliminarEvaluacion() {
    swal(
         'Estimado Usuario',
         'El módulo de evaluaciones de estándares mínimos no permite Eliminar Actividades, solo las puede editar, adicionar y cerrar',
         'warning'
         )
