...
function obtenerNombreArchi() {
    var texto = document.getElementById('CargarImagen').files[0].name;
    $("#fichero").text(texto)
}

//Funcion para Ver la informacion Adjunta del organigrama de la Eemmpresa.

function mostrarModalPdf() {
    
    $.ajax({
        type: 'GET',
        url: '/Organigrama/OrganigramaPdfMod',
        traditional: true,
        success: function (result) {
            if (result.Data = !null) {
                var nit = result[0];
                var extensiones33 = result[1];
                var extensiones = extensiones33.substring(extensiones33.lastIndexOf("."));
                console.log(extensiones)
                if (extensiones == ".pdf") {
                    $('#ModalPDF1111').modal('show');
                    $("#imagen").attr("src", "/AlisstaFile/" + nit + "/" + extensiones33);
                }
                //if (extensiones == ".jpg" || extensiones == ".png") {
                //    $('#ModalImagen').modal('show');
                //    $("#img").attr("src", "/D:/AlisstaFile/M_Empresa/O_GovO/O_Organigrama" + result);
                //}
            }
        }
    });
