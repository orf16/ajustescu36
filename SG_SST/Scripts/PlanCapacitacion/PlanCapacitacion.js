...
function ObtenerActividadporSede() 
{
    $.ajax({
        ...
        success: function (result) {
            ...
            tHtml += '<th width="10%" '+class_css_header+'>Acciones</th>';
            tHtml += '</tr></thead>';
			var cont = 0;
            $.each(result, function (key, val) {
                tHtml += '<tbody><tr>';
                ...
                for (var i = 1; i <= 12; i++) {
                    if (i == mes) {
						...
						tHtml += '<div style="background-color:#009900;"><font color="#FFFFFF">Actividad : '+ GetAct(val.fk_id_tipo_actividad) +' - '+ val.fecha_programada + ' ' + val.hora_inicio + ' - ' + val.hora_fin + '</font></div>';
						tHtml += '</td>';
                    }
...
function GetAct(val)
{
	switch(val)
	{
		case 1: return "Capacitación";
		break;
		case 2: return "Entrenamiento";
		break;
		case 3: return "Inducción/RE inducción";
		break;
	}
}
