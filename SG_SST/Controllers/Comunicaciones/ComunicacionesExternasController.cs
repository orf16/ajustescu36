...
namespace SG_SST.Controllers.Comunicaciones
{
    public class ComunicacionesExternasController : BaseController
    {
        ...
        [HttpGet]
        [ValidateInput(false)]
        public JsonResult GuardarComunicado(int? PK_Id_Comunicado, string Titulo, string Asunto, string CuerpoMensaje, string arreglo, string Origen, bool esEncuesta) 
        {
            var usuarioActual = ObtenerUsuarioEnSesion(System.Web.HttpContext.Current);
        ...
        public JsonResult obtenerCargos(string Busqueda)
        {
            try
            {
                ...
                if (Busqueda != "")
                {
                    ...
                    {
                        if (cargo.Contains(Busqueda.ToUpper()))
                        {
                ...
                return Json(cargosSiarp, JsonRequestBehavior.AllowGet);
                }
