...
namespace SG_SST.Controllers.Planificacion
{
    ...
    public class PlanTrabajoAnualController : BaseController
    {
        ...
        private string ObtenerObjetivosById(string pk_id_objetivo)
        {
            var usuarioActual = ObtenerUsuarioEnSesion(System.Web.HttpContext.Current);
            int pkobjectivo = int.Parse(pk_id_objetivo);
            List<SelectListItem> objetivos = new List<SelectListItem>();
            var objetivo = db.Tbl_Objetivos_SST.Where(x => x.FK_Id_Empresa == usuarioActual.IdEmpresa && x.PK_Id_Objetivo_Empresa == pkobjectivo).SingleOrDefault();
        ...
        public JsonResult ObtenerActividadPorPlan(int pkPlanEmpresa)
        {
            ...
            var planempresaactividad = db.Tbl_Plan_Empresa_Actividad.Where(x => (x.pk_plan_empresa == pkPlanEmpresa && x.NitEmpresa==usuarioActual.NitEmpresa))
                ...
                (a, b) => new { pk_id_actividad = a.pk_id_actividad, pk_id_plan_empresa = a.pk_plan_empresa, Estado = a.Estado, a.FechaProg, a.FechaReProg, a.FechaEje, a.HoraProgIni, a.HoraProgFin, b.FechaDesde, b.FechaHasta, b.Vigencia, actividad = a.Actividad });
            int progt = 0;
