...
namespace SG_SST.Controllers.LiderazgoGerencial
{
    ...
    public class PresupuestoController : BaseController
    {
        ...
        public ActionResult EliminarActividad(int pkActividad,string ipUsuario)
        {
            var usuarioActual = ObtenerUsuarioEnSesion(System.Web.HttpContext.Current);
            if (usuarioActual == null)
            {
                ...
                {
                    success = true,
                    mesansaje = "usuario no autorizado",
                }
               , JsonRequestBehavior.AllowGet);
            }
            EDInformacionAuditoria edInfoauditoria = new EDInformacionAuditoria()
            ...
            bool restpuestaGuardado = presupuestoServicios.EliminarActividad(pkActividad, edInfoauditoria);
            string mensaje = "";
