...
namespace SG_SST.ServiceRequest
{
    public class ServiceClient
    {
        ...
        public static Boolean PeticionesPostJsonRestFulRespuetaBoolEval<T>(string urlBaseServicio, string nombreCapacidad, object objeto)
        {
            Boolean respuesta;
            Uri baseUrl = new Uri(urlBaseServicio);
            var cliente = new RestClient
        ...
        public static int PeticionesPostJsonRestFulRespuetaint(string urlBaseServicio, string nombreCapacidad, RestSharp.Method tipoPeticion)
        {
            int respuesta;
            Uri baseUrl = new Uri(urlBaseServicio);
            var cliente = new RestClient
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                var respuestastring = response.Content;
                int parse = 0;
                if (int.TryParse(respuestastring, out parse))
                {
                    respuesta = parse;
                }
                else
