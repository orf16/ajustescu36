...
namespace SG_SST.Logica.Planificacion
{
    ...
    public class LNCarguePerfilSocioDemografico
    {
        ...
        public EDCarguePerfil CargarPlantillaCarguePlanificacion(EDCarguePerfil cargueP)
        {
            ...
            {
                registraLog.RegistrarError(typeof(LNCargue), string.Format("Error en el método CargarPlantillaPerfil  {0}: {1}", DateTime.Now, ex.StackTrace), ex);
        ...
        private bool ValidarNombreColumnas(ExcelWorksheet sheet)
        {
            if (sheet.Cells[1, 1].Value.Equals("Número Identificación") && sheet.Cells[1, 2].Value.Equals("Grado De Escolaridad")
                ...
                && sheet.Cells[1, 14].Value.Equals("FechaUltimoCargo") && sheet.Cells[1, 15].Value.Equals("Características Físicas para el desempeño del cargo") && sheet.Cells[1, 16].Value.Equals("Características Psicológicas para el desempeño del cargo")
