...
namespace SG_SST.Aplicacion.Servicios.Controllers
{
    ...
    public class CriteriosDeSSTController : ApiController
    {
        ...
        public HttpResponseMessage EliminarProductoCriterio(int idProducto, EDInformacionAuditoria edInfoauditoria)
        {
            try
            {
                var logica = new LNCriteriosSST();
                bool result = logica.EliminarProductoCriterio(idProducto, edInfoauditoria);
                if (result)
                {
                    var response = Request.CreateResponse(HttpStatusCode.OK, result);
                    return response;
