...
namespace SG_SST.Repositorio.MedicionEvaluacion
{
    public class PlanDeAccionManager : IPlanDeAccion
    {
                ...
                using (SG_SSTContext db1 = new SG_SSTContext())
                {
                    ...
                    if (ListaMod != null)
                    {
                        foreach (var item in ListaMod)
                        {
                            ...
                            {
                                ListaPlan = ObtenerListaActividadEvaluacion(empresa, item.Pk_Id_ModuloPlanAccion, item.Modulo);
                                if (ListaPlan != null)
                                {
        ...
        /// <returns>List<EDPlanDeAccion></returns>
        public List<EDPlanDeAccion> ObtenerListaActividadEvaluacion(int empresa, int Pk_Id_ModuloPlanAccion, string Modulo)
        {
            try
            {
                numeroActividadInsp = false;
                ...
                using (SG_SSTContext db1 = new SG_SSTContext())
                {
                    ...
                    List<int> vigencia = (from e in db1.Tbl_EvaluacionEmpresa

                                          where e.Fk_Id_Empresa == empresa

                                          select e.Vigencia).ToList().Distinct().ToList();




                    int idEmpresaEvaluar = empresaEvaluar.Pk_Id_Empresa_Evaluar;
                    foreach (var var in vigencia)
                    {
                        Actividades = (from eval in db1.Tbl_Evaluacion_Estandares_Minimos
                                       ...
                                       //Nueva linea
                                       join ee in db1.Tbl_EvaluacionEmpresa on eval.Fk_Id_EvaluacionEmpresa equals ee.Pk_Id_EvaluacionEmpresa
                                       where eval.Fk_Id_Empresa_Evaluar == idEmpresaEvaluar && ee.Vigencia == var && ae.EvaluacionCerrado == true
                                       select new EDActividadPlanDeAccion
                                       {
                                           ...
                                           //Observaciones = null
                                           //  actividadReporte="NA"
                                           actividadReporte = ee.Vigencia.ToString()
                    ...
                    var log = new RegistraLog();
                    log.RegistrarError(typeof(PlanDeAccionManager), string.Format("Se obtuvo los planes de acción del módulo planes de acción: {0}, {1}", DateTime.Now, empresa), new Exception());
            ...
            {
                var log = new RegistraLog();
                log.RegistrarError(typeof(PlanDeAccionManager), string.Format("Error obteniendo los planes de acción del módulo planes de acción: {0}, {1}. Error: {2}", DateTime.Now, empresa, ex.StackTrace), ex);
                return null;
            }
        }
        /// <summary>
        ...
        public bool EliminarActividadAccion(EDActividadPlanDeAccion actividadPlanDeAccion)
        {
            try
            {
                using (SG_SSTContext db1 = new SG_SSTContext())
                {
                    //Modificado por Javier García (Kerocorp) 11/04/2018
                    int idaccion = 0;
                    //FIN Modificado por Javier García (Kerocorp) 11/04/2018
                    ActividadAccion actividadAccion = new ActividadAccion();
                    actividadAccion = db1.Tbl_ActividadAccion.Where(x => x.Pk_Id_Actividad == actividadPlanDeAccion.Fk_Id_Actividad).FirstOrDefault();
                    ...
                    db1.SaveChanges();
                    idaccion = actividadAccion.Fk_Id_Accion;
                    //Modificado por Javier García (Kerocorp) 11/04/2018
                    if (actividadAccion.Fk_Id_Accion != 0)
                    {
                        VerificarEstado(actividadAccion.Fk_Id_Accion);
                    }
                    //FIN Modificado por Javier García (Kerocorp) 11/04/2018
        ...
        public bool AdicionarActividadEvaluacion(EDActividadPlanDeAccion actividadPlanDeAccion)
        {
            try
            {
                using (SG_SSTContext db1 = new SG_SSTContext())
                {
                    ...
                    actividadEvaluacion.EvaluacionCerrado = true;
        ...
        public List<EDPlanDeAccion> ConsultarListaPlanDeAccion(int nit, int Pk_Id_ModuloPlanAccion, string fechaInicial, string fechaFinal)
        {
            try
            {
                ...
                using (SG_SSTContext db1 = new SG_SSTContext())
                {
                    ...
                    {
                        ListaPlan = ConsultarListaActividadEvaluacion(empresa, Pk_Id_ModuloPlanAccion, modulo.Modulo, inicial, final);
                        if (ListaPlan != null)
                        {
        ...
        public List<EDPlanDeAccion> ConsultarListaActividadEvaluacion(int empresa, int Pk_Id_ModuloPlanAccion, string Modulo, DateTime fechaInicial, DateTime fechaFinal)
        {
            try
            {
                numeroActividadInsp = false;
        ...
        //Creado por Javier García (Kerocorp) 11/04/2018
        public void VerificarEstado(int IdAccion)
        {
            Accion Accion = new Accion();
            using (SG_SSTContext db = new SG_SSTContext())
            {
                var accionvar = (from s in db.Tbl_Acciones
                                    where s.Pk_Id_Accion == IdAccion
        ...
        //FIN Creado por Javier García (Kerocorp) 11/04/2018
    }
