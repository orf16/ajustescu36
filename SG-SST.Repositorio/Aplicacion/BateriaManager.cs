...
namespace SG_SST.Repositorio.Aplicacion
{
    public class BateriaManager : IBateria
    {
        ...
        public EDBateriaGestion ConsultarGestion(int IdGestion, int IdEmpresa)
        {
            ...
            using (SG_SSTContext db = new SG_SSTContext())
            {
                ...
                if (Listavar != null)
                {
                    ...
                    {
                        bool cantidadrespuestas = false;
                        var Listavar2 = (from s in db.Tbl_BateriaResultado
                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                         select s).ToList<BateriaResultado>();
                        if (EDBateriaGestion.Fk_Id_Bateria == 1 & EDBateriaGestion.bateriaExtra == 0)
                        {
                            if (Listavar2.Count >= 114)
                            {
                                cantidadrespuestas = true;
                            }
                        }
                        if (EDBateriaGestion.Fk_Id_Bateria == 2 & EDBateriaGestion.bateriaExtra == 0)
                        {
                            if (Listavar2.Count >= 97)
                            {
                                cantidadrespuestas = true;
                            }
                        }
                        if (EDBateriaGestion.Fk_Id_Bateria == 1 && item.NumeroIntentos == 1)
                        {
                            if (Listavar2.Count >= 31)
                            {
                                cantidadrespuestas = true;
                            }
                        }
                        if (EDBateriaGestion.Fk_Id_Bateria == 2 && item.NumeroIntentos == 1)
                        {
                            if (Listavar2.Count >= 31)
                            {
                                cantidadrespuestas = true;
                        ...
                        EDBateriaUsuario EDBateriaUsuario = new EDBateriaUsuario();
                        EDBateriaUsuario.Terminado = cantidadrespuestas;
                        EDBateriaUsuario.Nombre = item.Nombre;
                        EDBateriaUsuario.NumeroIdentificacion = item.NumeroIdentificacion;
                        EDBateriaUsuario.TipoDocumento = item.TipoDocumento;
                        EDBateriaUsuario.CorreoElectronico = item.CorreoElectronico;
        ...
        public EDBateriaGestion ConsultarGestionResultadoGeneral(int IdGestion, int IdEmpresa)
        {
            EDBateriaGestion EDBateriaGestion = new EDBateriaGestion();
            BateriaGestion BateriaGestion = new BateriaGestion();
            List<EDBateriaUsuario> ListaUsuarios = new List<EDBateriaUsuario>();
                ...
                if (Listavar != null)
                {
                    ...
                    foreach (var item in Listavar1)
                    {
                        ...
                        var Listavar2 = (from s in db.Tbl_BateriaResultado
                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                         select s).ToList<BateriaResultado>();
                        if (EDBateriaGestion.Fk_Id_Bateria == 1 && item.NumeroIntentos == 0)
                        {
                            if (Listavar2.Count>=105)
                            {
                                cantidadrespuestas = true;
                            }
                        }
                        if (EDBateriaGestion.Fk_Id_Bateria == 2 && item.NumeroIntentos == 0)
                        {
                            if (Listavar2.Count >= 88)
                            {
                                cantidadrespuestas = true;
                            }
                        }
        ...
        public EDBateriaInicial ConsultarInicialSoloIntra(int PkIdUsuario, int FkEmpresa)
        {
            EDBateriaInicial EDBateriaInicial = new EDBateriaInicial();
            int idusuarioconsultar = -1;
            ...
            using (SG_SSTContext db = new SG_SSTContext())
            {
                ...
                if (Listavar != null)
                {
                    var Listavar1 = (from s in db.Tbl_BateriaUsuario
                                    join d in db.Tbl_BateriaGestion on s.Fk_Id_BateriaGestion equals d.Pk_Id_BateriaGestion
                                    where s.TokenPrivado== Listavar.TokenPrivado && d.Fk_Id_Empresa == FkEmpresa
                                    select s).ToList<BateriaUsuario>();

                    foreach (var item in Listavar1)
                    {
                        if (item.NumeroIntentos==0)
                        {
                            idusuarioconsultar = item.Pk_Id_BateriaUsuario;
                        }
                    }
                }
            ...
            if (idusuarioconsultar != -1)
            {
                EDBateriaInicial = ConsultarInicialKey(idusuarioconsultar);
            }
        ...
        public void VerificarEncuestas(int IdGestion, int FkEmpresa)
        {
            string mensaje = string.Empty;
            using (SG_SSTContext db = new SG_SSTContext())
            {
                var Listavar = (from s in db.Tbl_BateriaGestion
                                where s.Fk_Id_Empresa == FkEmpresa && s.Pk_Id_BateriaGestion== IdGestion
                                select s).FirstOrDefault<BateriaGestion>();
                if (Listavar != null)
                {
                    var Listavar1 = (from s in db.Tbl_BateriaUsuario
                                    where s.Fk_Id_BateriaGestion == IdGestion && s.RegistroOperacion=="Fin" && s.ConfirmacionParticipacion != null
                                    select s).ToList<BateriaUsuario>();

                    foreach (var item in Listavar1)
                    {
                        mensaje = string.Empty;
                        var Listavar2 = (from s in db.Tbl_BateriaInicial
                                        where s.Fk_Id_BateriaUsuario== item.Pk_Id_BateriaUsuario
                                         select s).FirstOrDefault<BateriaInicial>();
                        if (Listavar2!=null)
                        {
                            if (Listavar2.TipoCargo==null)
                            {
                                //No tiene tipo cargo
                                mensaje += " "+ "Inicial:No tiene tipo cargo";
                            }
                        }
                        else
                        {
                            //No tiene bateria inicial
                            mensaje += " " + "Inicial:No tiene bateria inicial";
                        }

                        int tipobateria = Listavar.Fk_Id_Bateria;
                        int extra = item.NumeroIntentos;

                        if (tipobateria==1 && extra==0)
                        {
                            if (item.CheckPag9==null)
                            {
                                //check de la pagina 9 debería tener alguna respuesta
                                mensaje += " " + "usuario:check de la pagina 9 debería tener alguna respuesta";
                            }
                            else
                            {
                                if (item.CheckPag10 == null)
                                {
                                    //check de la pagina 10 debería tener alguna respuesta
                                    mensaje += " " + "usuario:check de la pagina 10 debería tener alguna respuesta";
                                }
                                else
                                {
                                    if (item.CheckPag9=="Si" && item.CheckPag10 == "Si")
                                    {
                                        var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                         select s).ToList<BateriaResultado>();
                                        if (Listavar3.Count < 123)
                                        {
                                            //Número incorrecto de respuestas
                                            mensaje += " " + "resultados:Número incorrecto de respuestas";
                                        }
                                    }
                                    if (item.CheckPag9 == "No" && item.CheckPag10 == "Si")
                                    {
                                        var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                         select s).ToList<BateriaResultado>();
                                        if (Listavar3.Count < 114)
                                        {
                                            //Número incorrecto de respuestas
                                            mensaje += " " + "resultados:Número incorrecto de respuestas";
                                        }
                                    }
                                    if (item.CheckPag9 == "Si" && item.CheckPag10 == "No")
                                    {
                                        var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                         select s).ToList<BateriaResultado>();
                                    ...
                                    if (item.CheckPag9 == "No" && item.CheckPag10 == "No")
                                    {
                                        var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                         where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                         select s).ToList<BateriaResultado>();
                                        if (Listavar3.Count < 105)
                                        {
                                            //Número incorrecto de respuestas
                                            mensaje += " " + "resultados:Número incorrecto de respuestas";
                                        }
                        ...
                        if (tipobateria == 2 && extra == 0)
                        {
                            ...
                            {
                                if (item.CheckPag9=="Si")
                                {
                                    var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                     where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                     select s).ToList<BateriaResultado>();
                                    if (Listavar3.Count < 97)
                                    {
                                        //Número incorrecto de respuestas
                                        mensaje += " " + "resultados:Número incorrecto de respuestas";
                                    }
                                }
                                if (item.CheckPag9 == "No")
                                {
                                    var Listavar3 = (from s in db.Tbl_BateriaResultado
                                                     where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                                     select s).ToList<BateriaResultado>();
                                    if (Listavar3.Count < 88)
                                    {
                                        //Número incorrecto de respuestas
                                        mensaje += " " + "resultados:Número incorrecto de respuestas";
                                    }
                        ...
                        if (tipobateria == 1 && extra == 1)
                        {
                            var Listavar3 = (from s in db.Tbl_BateriaResultado
                                             where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                             select s).ToList<BateriaResultado>();
                            if (Listavar3.Count < 31)
                            {
                                //Número incorrecto de respuestas
                                mensaje += " " + "resultados:Número incorrecto de respuestas";
                            }
                        }
                        if (tipobateria == 2 && extra == 1)
                        {
                            var Listavar3 = (from s in db.Tbl_BateriaResultado
                                             where s.Fk_Id_BateriaUsuario == item.Pk_Id_BateriaUsuario
                                             select s).ToList<BateriaResultado>();
                        ...
                        //Tiene fecha respuesta
                        if (item.FechaRespuesta==null)
                        {
                            //No tiene Fecha respuesta
                            mensaje += " " + "usuario:Sin fecha de respuesta";
                        }
                        ...
                        {
                            if (mensaje!= item.RegistroOperacionExtra)
                            {
                                item.RegistroOperacionExtra = mensaje;
                                db.Entry(item).State = EntityState.Modified;
                                try
                                {
                                    db.SaveChanges();
        ...
        public void VerificarEncuestasAdministrador()
        {
            string mensaje = string.Empty;
            using (SG_SSTContext db = new SG_SSTContext())
            {
                ...
                if (Listavar != null)
                {
                    ...
                    var Listavar1 = (from s in db.Tbl_BateriaUsuario
                                     where s.Fk_Id_BateriaGestion == item1.Pk_Id_BateriaGestion && s.RegistroOperacion == "Fin" && s.ConfirmacionParticipacion != null
                                     select s).ToList<BateriaUsuario>();
                    ...
                    foreach (var item in Listavar1)
                    {
                        ...
                        int tipobateria = item1.Fk_Id_Bateria;
                        int extra = item.NumeroIntentos;
