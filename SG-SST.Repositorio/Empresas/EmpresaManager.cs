...
namespace SG_SST.Repositorio.Empresas
{
    public class EmpresaManager : IEmpresa
    {
        ...
        /// <returns></returns>
        public List<EDEmpresas> ObtenerEmpresasRegistradasPorPrefijo(string prefijo)
        {
            try {
                var context = new SG_SSTContext();
                return context.Tbl_Empresa.Where(e => e.Razon_Social.Contains(prefijo)).Select(e => new EDEmpresas()
                {
                    Id_Empresa = e.Pk_Id_Empresa,
                    Razon_Social = e.Razon_Social,
                    Nit_Empresa = e.Nit_Empresa
            ...
            catch { return new List<EDEmpresas>(); }
