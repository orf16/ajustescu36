...
namespace SG_SST.Repositorio.Participacion
{
    public class ReporteManager : IReporte
    {
        ...
        public List<EDGrupo> ObtenerGruposPorComunicacion (string nit)
        {
            List<EDGrupo> grupos = new List<EDGrupo>();
            using (SG_SSTContext contex = new SG_SSTContext())
            {
                grupos=(from gruUC in contex.Tbl_GrupoUsuariosComunicaciones 
                        join gruC in contex.Tbl_GruposComuniciones on gruUC.fk_id_grupo_comunicaciones equals gruC.pk_id_grupo
                      //  where gruC.NitEmpresa==nit

                        select new EDGrupo
                        {
                            IDGrupo=gruC.pk_id_grupo,
                            NombreGrupo=gruC.NombreGrupo
                        }).ToList();
        ...
        public List<EDCargo> ObtenerCargosPorComunicacion(string nit)
        {
            List<EDCargo> cargos = null;
            using (var context = new SG_SSTContext())
            {
                cargos=context.Database.SqlQuery<EDCargo>("SP_Cargos_Comunicaciones @nit",
                    
                      new SqlParameter("@nit", nit)).ToList();
                    
            }
            return cargos;
        }

        public List<EDCargo> ObtenerCargosAPPPorComunicacion(string nit)
        {
            List<EDCargo> cargosAPP = null;
            using (var context = new SG_SSTContext())
            {
                cargosAPP = context.Database.SqlQuery<EDCargo>("SP_comunicaciones_appCargos @nit",
        ...
        public bool ValidarExistePlanAccionCondicionesInseguras(int idReporte)
        {
            ...
            try
            {
                using (var context = new SG_SSTContext())
                {
                    plan = (from u in context.Tbl_Actividad_Plan_Accion
                            where u.Fk_Id_ModuloPlanAccion == 5 && u.Fk_Plan_Inspección == idReporte
                            select u
                               ).FirstOrDefault();
            ...
            {
                registraLog.RegistrarError(typeof(ReporteManager), string.Format("Error al validar existencia de plan de acción cerrado de  reporte de condiciones inseguras  {0}: {1}", DateTime.Now, ex.StackTrace), ex);
            }
            return cerrado;

        }

        public bool EliminarReporteCondicionesInseguras(int idReporte)
        {
            using (SG_SSTContext context = new SG_SSTContext())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    ...
                    try
                    {
                        Reporte reporte = context.Tbl_Reportes.Find(idReporte);
                        context.Tbl_Reportes.Remove(reporte);
                        context.SaveChanges();
                        transaction.Commit();
                        return true;
                    }
